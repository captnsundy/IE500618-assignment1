...
The training set was split from the original dataset using a random 80/20 split. The test set was kept separate to ensure that the model's performance could be evaluated on unseen data, without which we wouldn't be able to assess how well the model generalizes to new data. This helps prevent overfitting and provides a more accurate measure of the model's predictive capabilities.
